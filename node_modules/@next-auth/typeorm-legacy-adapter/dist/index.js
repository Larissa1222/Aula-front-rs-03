...
var _errors = require("next-auth/errors");
...
const Adapter = (typeOrmConfig, options = {}) => {
  ...
  const typeOrmConfigObject = typeof typeOrmConfig === "string" ? _config.default.parseConnectionString(typeOrmConfig) : typeOrmConfig; // Load any custom models passed as an option, default to built in models
  ...
  const config = _config.default.loadConfig(typeOrmConfigObject, { ...options,
    models
  ...
  async function getAdapter(appOptions) {
    var _appOptions$logger;

    const logger = (_appOptions$logger = appOptions.logger) !== null && _appOptions$logger !== void 0 ? _appOptions$logger : console; // Helper function to reuse / restablish connections
    // (useful if they drop when after being idle)

    async function _connect() {
      // Get current connection by name
      connection = (0, _typeorm.getConnection)(config.name); // If connection is no longer established, reconnect
    ...
    const {
      manager
    } = connection; // Display debug output if debug option enabled
    ...
    if (config.type === "mongodb") {
      idKey = "_id"; // Using a dynamic import causes problems for some compilers/bundlers
    ...
    const sessionMaxAge = appOptions && appOptions.session && appOptions.session.maxAge ? appOptions.session.maxAge * 1000 : defaultSessionMaxAge;
    const sessionUpdateAge = appOptions && appOptions.session && appOptions.session.updateAge ? appOptions.session.updateAge * 1000 : 0;
    ...
    async function getUser(id) {
      debug("GET_USER", id); // In the very specific case of both using JWT for storing session data
    ...
    async function deleteUser(userId) {
      debug("DELETE_USER", userId); // @TODO Delete user from DB
    ...
    async function unlinkAccount(userId, providerId, providerAccountId) {
      debug("UNLINK_ACCOUNT", userId, providerId, providerAccountId); // @TODO Get current user from DB
    ...
    async function deleteSession(sessionToken) {
      ...
      try {
        return await manager.delete(Session, {
          sessionToken
        });
      } catch (error) {
    ...
    async function createVerificationRequest(identifier, url, token, secret, provider) {
      ...
      try {
        ...
        const {
          ...
          maxAge
        } = provider; // Store hashed token (using secret as salt) so that tokens cannot be exploited
        // even if the contents of the database is compromised.
        // @TODO Use bcrypt function here instead of simple salted hash

        const hashedToken = (0, _crypto.createHash)("sha256").update(`${token}${secret}`).digest("hex");
        let expires = null;
        ...
        const verificationRequest = await manager.save(newVerificationRequest); // With the verificationCallback on a provider, you can send an email, or queue
